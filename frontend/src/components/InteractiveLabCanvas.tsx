...
const drawBeaker = (
  ...
  if (currentVol > 0) {
    ...
    ctx.beginPath();
    ctx.roundRect(x + 2, liquidY, w - 4, y + h - radius - liquidY, [
      0,
      0,
      radius,
      radius,
...
const drawCylinder = (
  ...
  ctx.save();
  // If pouring, tilt the cylinder
  if (isPouring) {
    ctx.translate(x + w / 2, y);
    ctx.rotate(Math.PI / 3.5); // Tilt ~51 degrees
    ctx.translate(-(x + w / 2), -y);
  }
  ...
  if (currentVol > 0) {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(x, liquidY);
    ctx.lineTo(x + w, liquidY);
    ctx.stroke();
  }
...
const InteractiveLabCanvas: React.FC<InteractiveLabCanvasProps> = ({
  ...
  const drawApparatus = useCallback(
      ...
      apparatus.forEach((item) => {
        ...
        if (item.data?.isPouring && item.data.pouringTargetId) {
          ...
          if (target) {
            ...
            // Get source liquid color
            const sourceColor =
              item.data?.liquidColor || "rgba(56, 189, 248, 0.8)";
            ...
            const midX =
              (startX + endX) / 2 + Math.sin(progress * Math.PI) * 15;
            const midY = (startY + endY) / 2 - 20;

            // Draw main stream with varying width
            const streamWidths = [8, 6, 4];
            streamWidths.forEach((width, index) => {
              ctx.strokeStyle = sourceColor.replace(
                "0.8",
                (0.8 - index * 0.2).toString(),
              );
              ctx.lineWidth = width;
              ctx.lineCap = "round";
              ctx.lineJoin = "round";
            ...
            // Draw flowing particles along stream
            ...
            for (let i = 0; i < numParticles; i++) {
              ...
              // Draw particle with opacity gradient
              const opacity = Math.sin(particleProgress * Math.PI) * 0.8;
              ctx.fillStyle = sourceColor.replace("0.8", opacity.toFixed(2));
              ctx.beginPath();
              ctx.arc(particleX, particleY, 2.5, 0, Math.PI * 2);
              ctx.fill();
            }

            // Impact/splash zone at target
            const splashSize = Math.sin(progress * Math.PI) * 8 + 6;

            // Splash rings
            for (let ring = 1; ring <= 3; ring++) {
              const ringRadius = splashSize * ring;
              ctx.strokeStyle = sourceColor.replace(
                "0.8",
                (0.6 / ring).toFixed(2),
              );
            ...
            // Main splash pool
            ctx.fillStyle = sourceColor.replace("0.8", "0.6");
            ctx.beginPath();
            ctx.arc(endX, endY, splashSize, 0, Math.PI * 2);
            ctx.fill();
        ...
        if (type === "beaker") {
          ...
          drawBeaker(ctx, x, y, width, height, maxVol, currentVol, liquidColor);
    ...
    [canvasSize, apparatus, benchHeight, tableTotalWidth, hoveredId],
  );
